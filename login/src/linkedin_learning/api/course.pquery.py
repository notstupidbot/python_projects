...
def convert2Xml(data, page_name):
    ...
    doc=pq(xml_content)
    # doc=BeautifulSoup(xml_content,'html.parser')
    return doc
...
def getVideoMeta(v_status_urn, doc):
    ...
    for el in v_status_lookup:
        el_nd = doc(el).parent()
        if el_nd('entitytype').text() == 'VIDEO':
            break_the_loop = True
...
def getCourseToc(item_star,doc,course_slug):
    ...
    if toc_nd:
        ...
        # break_loop = False
        # if not video_urn:
        #     toc_nd = doc('star_lyndaVideoViewingStatus:contains("%s")' % item_star)
        #     for toc_el in toc_nd:
        #         toc_el_nd = doc(toc_el)
        #         if toc_el_nd.text() == item_star:
        #             # if toc_el_nd.parent()("entityType") == "video":
        #             #     toc_nd = toc_el_nd.parent()
        #             toc_el_nd_p = toc_el_nd.parent()
        #             if toc_el_nd_p("entityType").text() == 'VIDEO':
        #                 # print(toc_el_nd_p)
        #                 toc_nd = toc_el_nd_p
        #                 break_loop=True
        #         if break_loop:
        #             break    
        #     video_urn = toc_nd("entityUrn").text()
        if video_urn:
            # print(video_urn)
            break_the_loop=False
